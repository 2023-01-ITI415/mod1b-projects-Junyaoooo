...
public class FishControl : MonoBehaviour
{
    ...
    //now fish Direction
    private FishDirection currentDir1 = FishDirection.Left;
    ...
    {
        int scalX1 = currentDir1 == FishDirection.Left ? -1 : 1;
        transform.localScale=new Vector3(scalX1, 1, 1);

        //random y from 0 to 1:
        float randomY = Random.value;
        //fish head setting
        if (currentDir1 == FishDirection.Left)
        {
    ...
    void Update()
    {
        ...
        if (Vector3.Distance(transform.position,target1)<0.06f) 
        {
            ...
            currentDir1= currentDir1==FishDirection.Left?FishDirection.Right:FishDirection.Left;
            //end point
            SetTarget();
        }
    }
